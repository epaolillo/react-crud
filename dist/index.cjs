...
const Login = () => {
  ...
  const [form, setForm] = require$$0.useState({
    email: "",
    password: "",
    remember: false
  });
  const [errors, setErrors] = require$$0.useState({});
  const [alert, setAlert] = require$$0.useState({
    show: false,
    type: "error",
    message: ""
  });
...
const AffiliateContext = require$$0.createContext();
const useAffiliate = () => {
  const context = require$$0.useContext(AffiliateContext);
  if (!context) {
...
const AffiliateProvider = ({ children }) => {
  const [currentAffiliate, setCurrentAffiliate] = require$$0.useState(null);
  const [userAffiliates, setUserAffiliates] = require$$0.useState([]);
...
const Navbar = ({
  ...
  const [affiliateDropdownOpen, setAffiliateDropdownOpen] = require$$0.useState(false);
  const [userDropdownOpen, setUserDropdownOpen] = require$$0.useState(false);
  const affiliateDropdownRef = require$$0.useRef(null);
  const userDropdownRef = require$$0.useRef(null);
  const {
    currentAffiliate,
    userAffiliates,
    switchAffiliate,
...
const EditorJs = require$$0.forwardRef(({
  data = { blocks: [] },
  ...
  const holderRef = require$$0.useRef(null);
  const holderId = require$$0.useRef(`editor-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`);
  const isInitializing = require$$0.useRef(false);
...
const DynamicStory = ({
  ...
  const [errors, setErrors] = require$$0.useState({});
  const [saving, setSaving] = require$$0.useState(false);
  const [asyncOptions, setAsyncOptions] = require$$0.useState({});
  const [loadingOptions, setLoadingOptions] = require$$0.useState({});
  const editorRef = require$$0.useRef(null);
  const {
    fields = [],
    entityType = "story",
...
const EntityTable = require$$0.forwardRef(({
  data = [],
  config = {},
  loading = false
}, ref) => {
  ...
  const [searchTerm, setSearchTerm] = require$$0.useState(config.currentSearch || "");
  const [filters, setFilters] = require$$0.useState(config.currentFilters || {});
  const [searchTimeout, setSearchTimeout] = require$$0.useState(null);
  const [showEditor, setShowEditor] = require$$0.useState(false);
  const [editingEntity, setEditingEntity] = require$$0.useState(null);
  const [editorLoading, setEditorLoading] = require$$0.useState(false);
  const {
  ...
  const debouncedSearch = require$$0.useCallback(() => {
    if (onSearch) {
...
const Admin = ({ entities = [], customRoutes = [] }) => {
  ...
  const [currentSection, setCurrentSection] = require$$0.useState("dashboard");
  const [mobileSidebarOpen, setMobileSidebarOpen] = require$$0.useState(false);
  require$$0.useEffect(() => {
    const checkAuth = async () => {
      try {
        const response = await fetch("/api/auth/me");
...
function createAutoEntity(config) {
  ...
  const EntityComponent = () => {
    ...
    const [loading, setLoading] = require$$0.useState(false);
    const [pagination, setPagination] = require$$0.useState(null);
    const [currentPage, setCurrentPage] = require$$0.useState(1);
    const [pageSize, setPageSize] = require$$0.useState(20);
    const [currentSearch, setCurrentSearch] = require$$0.useState("");
    const [currentFilters, setCurrentFilters] = require$$0.useState({});
...
function App({ entities = [] }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(ErrorBoundary, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(AffiliateProvider, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(reactRouterDom.BrowserRouter, { children: /* @__PURE__ */ jsxRuntimeExports.jsxs(reactRouterDom.Routes, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(reactRouterDom.Route, { path: "/login", element: /* @__PURE__ */ jsxRuntimeExports.jsx(Login, {}) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(reactRouterDom.Route, { path: "/admin/*", element: /* @__PURE__ */ jsxRuntimeExports.jsx(Admin, { entities }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(reactRouterDom.Route, { path: "/", element: /* @__PURE__ */ jsxRuntimeExports.jsx(reactRouterDom.Navigate, { to: "/admin", replace: true }) })
  ] }) }) }) });
}
exports.EntityService = EntityService;
exports.commonEntityGroups = commonEntityGroups;
exports.commonIcons = commonIcons;
exports.createAction = createAction;
exports.createAutoEntity = createAutoEntity;
exports.createColumn = createColumn;
exports.createEntity = createEntity;
exports.createEntityTableConfig = createEntityTableConfig;
